function submitWithConfiguration_func(conf)
  addpath('./lib/jsonlab');

  parts = parts_func(conf);
  ...
  try
    response = submitparts_func(conf, email, token, parts);
  catch
    e = lasterror();
    fprintf('\n!! Submission failed: %s\n', e.message);
    fprintf('\n\nFunction: %s\nFileName: %s\nLineNumber: %d\n', ...
...
function response = submitparts_func(conf, email, token, parts)
  body = makePostBody(conf, email, token, parts);
  submissionUrl = submissionUrl_func();
...
function [parts] = parts_func(conf)
  parts = {};
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function submissionUrl = submissionUrl_func()
